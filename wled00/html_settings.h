...
content="text/html; charset=windows-1252"><title>ConfiguraciC3n HITO</title>
<style>
body{text-align:center;background:#222;height:100;margin:0}html{--h:11.55vh}button{background:#333;color:#fff;font-family:Verdana,Helvetica,sans-serif;border:.3ch solid #333;display:inline-block;font-size:7vmin;height:var(--h);width:95%%;margin-top:2.4vh}
</style><script>
...
id="b">Volver</button></form><form action="/settings/wifi"><button 
type="submit">WiFi</button></form><form action="/settings/leds"><button 
type="submit">Preferencias</button></form><form action="/settings/ui"><button 
type="submit">Interfase</button></form><form action="/settings/sync"><button 
type="submit">Sync</button></form><form action="/settings/time"><button 
type="submit">Extra</button></form><form action="/settings/sec"><button 
type="submit">Seguridad</button></form></body></html>)=====";
...
content="width=500"><title>ConfiguraciC3n WiFi</title><script>
function B(){window.open("/settings","_self")}function GetV() {var d=document;
%CSS%%SCSS%</head><body onload="GetV()"><form id="form_s" 
name="Sf" method="post"><div class="helpB"><button type="button" onclick="H()">?
</button></div><button type="button" onclick="B()">Volver</button><button 
type="submit">Guardar</button><hr><h2>Config WiFi</h2><h3>Conectar a tu WiFi
</h3>Nombre del WiFi (SSID):<br><input name="CS" maxlength="32"><br>
Contrasena del WiFi:<br><input type="password" name="CP" maxlength="63"><br>
IP EstC!tica (0.0.0.0 para DHCP):<br><input name="I0" type="number" min="0" 
max="255" required> . <input name="I1" type="number" min="0" max="255" required>
 . <input name="I2" type="number" min="0" max="255" required> . <input 
name="I3" type="number" min="0" max="255" required><br>Puerta de enlace:<br>
<input name="G0" type="number" min="0" max="255" required> . <input name="G1" 
...
required><br>Mascara de red:<br><input name="S0" type="number" min="0" 
max="255" required> . <input name="S1" type="number" min="0" max="255" required>
 . <input name="S2" type="number" min="0" max="255" required> . <input 
name="S3" type="number" min="0" max="255" required><br><input type="checkbox" 
name="WS" type="hidden"><br><button type="button" onclick="B()">Volver</button>
